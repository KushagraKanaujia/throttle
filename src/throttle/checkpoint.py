...
class CheckpointManager:
    ...
    def validate_resume(
        ...
        Returns (first_incomplete_index, list_of_completed_positions, list_of_suspects).
        ...
        for cp_path in checkpoints:
            ...
            report_data = cp_data["position_report"]
            throughput = report_data["output_throughput_toks_per_s"]
            
            # Group by first character of position ('B' for baseline, 'C' for candidate)
            treatment_group = pos[0]
            treatment_groups.setdefault(treatment_group, []).append(throughput)

            raw_path = self.session_dir / report_data["raw_report_path"]
            if not raw_path.exists():
                raise CheckpointError(
                    f"Checkpoint for {pos} exists but "
                    f"raw report {raw_path} is missing."
                )

        # Run outlier checks to identify suspects
        suspects = self.detect_outliers(treatment_groups)
        ...
        return first_incomplete, completed_positions, suspects
    ...
    def detect_outliers(
        ...
        for position_prefix, throughputs in treatment_groups.items():
            ...
            # Avoid division by zero for identical runs
            if mad < 1e-9:
            ...
            for idx, val in enumerate(throughputs):
                ...
                if mod_z > 3.5:
                    # Map prefix ('B'/'C') back to its approximate position marker for reporting
                    suspects.append(f"{position_prefix}{idx + 1}")
        return suspects
